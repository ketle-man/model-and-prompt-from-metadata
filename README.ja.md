...
PNG 画像またはワークフロー JSON をノードにドロップするだけで、埋め込まれたメタデータからチェックポイント・VAE・プロンプトを抽出してノードに反映します。過去に生成した画像の設定を即座に再現でき、試行錯誤のサイクルを短縮します。
...
![ノードのスクリーンショット](docs/screenshot1.png)

*左: Model-Prompt from Metadata — チェックポイント・VAE・プロンプトが検出され自動選択。右: Model from Metadata — UNet 系モデル（QWEN）が検出され、ワークフローを直接使うよう案内。*
...
PNG または JSON をドロップしてチェックポイントと VAE を読み込むノードです。
...
| clip | CLIP | CLIP |
| vae | VAE | VAE（「なし」選択時はチェックポイント内蔵 VAE）|
...
チェックポイント・VAE に加えてポジティブ／ネガティブプロンプトも抽出・エンコードするノードです。
...
1. PNG 画像またはワークフロー JSON をノード上のドロップゾーンにドラッグ＆ドロップ（またはクリックしてファイル選択）。
...
5. **VAE が 1 件インストール済みの場合も自動選択**されます。ワークフローに VAELoader がない場合は「なし」が自動選択されます。手動で変更する場合はリストから選択してください。
6. プロンプトはクリックで選択し、下部のプレビューで全文を確認できます。
...
├── metadata_checkpoint_node.py   # Python ノード定義（2クラス）
└── js/
